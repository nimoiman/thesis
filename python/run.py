...
@command
def train(*args, bep=0.001, vector_dim=1, rate=0.125, seed=1234, lbg_eps=0.01,
          train_dir="training"):
...
def test(*args):
    ...
    with open(os.path.join(train_dir, "bit_allocs.pickle"), "rb") as f:
    ...
    for i in range(block_size):
        for j in range(block_size):
            ...
            # Test on vector covq
            subprocess.check_call(["../covq/covq", "--test", "_tmp_both.csv",
                                   os.path.join(train_dir, str(i)+str(j),
                                                "cb_both.csv"),
                                   os.path.join(train_dir, str(i)+str(j),
                                                "cwmap_both.csv"),
                                   "t_out_both.csv", "--bep", str(bep),
