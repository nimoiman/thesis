...
The Lloyd-Max and Linde-Buzo-Gray quantizer design algorithms will be discussed in this section. These two algorithms rely on the previously discuses conditions of optimality. A big advantage for these algorithms is that they do not require knowledge of the underlying source distribution, and can instead be applied to a training set taken from the source. Recall that an optimal quantizer can be defined by its codebook, since the encoder can be obtained from the codebook by applying the nearest neighbor condition.

Recall that the nearest neighbor condition optimizes the encoder for a fixed codebook, and the centroid condition optimizes the codebook for a fixed encoder. The Lloyd-Max algorithm iteratively applies the centroid and nearest neighbor conditions on a training set until the codebook converges. The Lloyd-Max algorithm is implemented using a training set $\mathcal{T}$ taken from the source distribution. An overview of the algorithm follows:
...
One limitation of the Lloyd-Max algorithm is that is does not guarantee convergence to an optimal quantizer. The performance quantizer is dependent on the initial state of the codebook. It is important that the codebook is initialized properly to ensure convergence onto a good codebook.

The Linde-Buzo-Gray can be used in conjunction with the Lloyd-Max algorithm to improve initialization conditions. In the LBG algorithm, the codebook initially contains a single code vectors placed at the centroid of the training set. Code vectors are iteratively split in two in between iterations of the Lloyd-Max algorithm until the final codebook size is obtained. A more detailed description of the algorithm follows:
...
The LBG algorithm can provide better initialization conditions and therefore a better quantizer. Convergence onto an optimal solution is still not guaranteed, however.
...
As mentioned in the Introduction, the system to be analyzed in this report consists of two transmitter that each transmit indices over a channel to a single receiver. The decoder at the receiver uses the received indices to estimate the two original sources. It is assumed that the two sources are dependent, so a joint decoder system which uses both indices to estimate each source can obtain better source estimates. Both noisy and noiseless channels will be analyzed in this report.

It is assumed that each of the two sources is scalar valued. Let $(X,Y)\in\real^2$ denote the random source which has a density function $f(x,y)$. The encoder $\mathcal{E}$ maps $(X,Y)$ onto a pair of indices $i$ and $j$:

\begin{equation*}
    \mathcal{E} : \real^2\to\{1,\ldots,N_X\} \times \{1,\ldots,N_Y\}
\end{equation*}
where $N_X$ and $N_Y$ are the number of output levels for the two encoders. If the encoding $X$ and $Y$ is performed independently, $\mathcal{E}$ can be expressed as two encoders
\begin{gather*}
...
Let $k$ denote the index received at the receiver corresponding to index $i$. Let $l$ denotes the index at the receiver corresponding to index $j$. The indices $k$ and $l$ are used to estimate $X$ and $Y$ by the decoder. Let 
\begin{equation}
...
\end{align}
denote the codebooks for $X$ and $Y$. Notice that $\mathcal{C} = \mathcal{C}_X \times \mathcal{C}_Y$ when decoding independently.

The sum of the squared-error distortion across both sources will be used to measure performance:
\begin{equation}
    D_{avg} = E[{(X-\hat{X})}^2 + {(Y-\hat{Y})}^2]
\end{equation}
where $(\hat X, \hat Y) \in \mathcal{C}$ represent the estimates of $X$ and $Y$ at the receiver.

In this report, different combinations of joint and independent encoders and codebook, over noisy and noiseless channel will be considered. The table below shows the notation that will be used for the different quantizers that will be studied:
...
The average distortion for the six systems will now be presented using explicit expressions. Doing so will shed light on the problem, while also helping in the derivation of new conditions of optimality for the \sysIJ\ and \sysIJN\ systems. These expressions are easily derived by conditioning on transmitted and received indices. 
...
\end{equation}
Unlike the previous system, the average distortion for the \sysIIN\ system can not be separated into two separate terms unless it is assumed that the two channels for the transmitted indices are independent: $P(k,l|i,j) = P(k|i)P(l|j)$. Under this assumption we have
\begin{multline}
...
\end{equation}
The above expression can be minimized using a 2-dimensional channel optimized quantizer with codebook size $N_XN_Y$. Note that the transmission indices are no longer associated with code vectors, like in~(\ref{eq:dist_indep_chan}), because code vectors are defined by two indices, not just one.
...
\end{equation}
Unlike the \sysII\ system, it is not possible to separate the above expression into two terms than can be independently minimized using two separate quantizers. Notice the slight difference between this expression and that of~(\ref{eq:dist_JJ}). New conditions for optimality will be derived for this system in the next subsection and the implementation of such a quantizer will be discussed in detail in the following section. 
